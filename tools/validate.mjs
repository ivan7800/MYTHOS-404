...
const kHasText=(v,min=3)=>typeof v==='string'&&v.trim().length>=min,kHasList=v=>Array.isArray(v)&&v.length>0,kGeneric=/^Entrada de descubrimiento\b/i;
function completeness(e={}){const status=knowledgeStatus(e);let score=0;if(kHasText(e.name))score+=4;if(kHasText(e.type))score+=3;if(kHasText(e.culture))score+=3;if(kHasText(e.tradition)||kHasText(e.era))score+=4;if(kHasText(e.region))score+=3;if(kHasText(e.period)||kHasText(e.era))score+=3;if(kHasText(e.summary,40)&&!kGeneric.test(e.summary))score+=15;if(kHasText(e.domain,12)&&!kGeneric.test(e.domain))score+=10;if(kHasList(e.relations))score+=10;if(kHasList(e.symbols))score+=5;if(kHasList(e.places))score+=5;const refs=e.sourceRefs||[];score+=status==='reviewed'?Math.min(15,8+Math.max(0,refs.length-1)*2):status==='documented'?8:refs.length?3:0;if(status==='documented'){if(kHasText(e.provenance?.entityType))score+=4;if(kHasText(e.provenance?.category))score+=3;}if(kHasList(e.passages))score+=8;if(kHasList(e.variants))score+=7;if(kHasList(e.myths)||kHasList(e.motifs))score+=5;return Math.max(0,Math.min(100,score));}
...
const required=['index.html','css/styles.css','js/core.js','js/index.js','js/store.js','js/app.js','manifest.webmanifest','sw.js','README.md','ARCHITECTURE.md','ACADEMIC_METHOD.md','COVERAGE_AND_ETHICS.md','THIRD_PARTY_DATA.md','QA_REPORT.md','AUDIT_REPORT.md'];
for(const f of required)exists(f)||fail(`Falta ${f}`);if(!errors.length)ok('Estructura Knowledge Edition v9 completa');
...
if(core&&index){
  ...
  if(fullEntities.length<5000)fail('La Knowledge Edition debe conservar más de 5.000 entidades');else ok(`Corpus mundial: ${fullEntities.length} entidades`);
  ...
  const reviewed=fullEntities.filter(e=>knowledgeStatus(e)==='reviewed'),documented=fullEntities.filter(e=>knowledgeStatus(e)==='documented'),discovery=fullEntities.filter(e=>knowledgeStatus(e)==='discovery'),rawDiscovery=fullEntities.filter(e=>e.reviewStatus==='discovery');
  if(reviewed.length!==index.counts.reviewed||documented.length!==index.counts.documented||discovery.length!==index.counts.discovery||rawDiscovery.length!==index.counts.externalDiscovery)fail('Conteos de madurez documental desincronizados');else ok(`${reviewed.length} revisadas + ${documented.length} documentadas externamente + ${discovery.length} discovery`);
  if(documented.length<100)fail('La capa documentada externamente quedó vacía o demasiado pequeña');else ok(`${documented.length} fichas con documentación externa A/B y revisión MYTHOS pendiente`);
  ...
  const reviewedSet=new Set(reviewed.map(e=>e.id)),bootSet=new Set(indexedReviewed.map(e=>e.id));for(const id of reviewedSet)if(!bootSet.has(id))fail(`Revisada ausente del bootstrap: ${id}`);for(const id of bootSet)if(!reviewedSet.has(id))fail(`Entrada no revisada filtrada incorrectamente en bootstrap: ${id}`);
  const scores=fullEntities.map(completeness);if(scores.some(x=>!Number.isFinite(x)||x<0||x>100))fail('Completitud fuera de rango 0–100');else ok(`Completitud documental válida · media ${Math.round(scores.reduce((a,b)=>a+b,0)/scores.length)}%`);
  for(const e of fullEntities){const rowStatus=knowledgeStatus(e);if(rowStatus==='reviewed'&&e.reviewStatus==='discovery')fail(`Discovery promovida accidentalmente a revisada: ${e.id}`)}
  if(!core.knowledgePolicy||core.knowledgePolicy.completenessNote?.length<30)fail('Falta MYTHOS Knowledge Standard en core');else ok('MYTHOS Knowledge Standard presente y explícito');
  ...
  const redirects=core.entityRedirects||{};for(const [from,to] of Object.entries(redirects)){if(entitySet.has(from))fail(`Redirect legacy todavía existe como entidad: ${from}`);if(!entitySet.has(to))fail(`Redirect apunta a entidad inexistente: ${from} -> ${to}`);if(redirects[to])fail(`Redirect encadenado no permitido: ${from} -> ${to}`)}if(Object.keys(redirects).length)ok(`${Object.keys(redirects).length} redirects legacy resuelven consolidaciones sin duplicar fichas`);
  const batch=core.knowledgeStats?.lastReviewBatch||{},bp=fullEntities.filter(e=>e.provenance?.mythosReview==='promoted-2026-08-13').length,be=fullEntities.filter(e=>e.provenance?.reviewEnrichment==='2026-08-13').length;if(batch.promoted!==bp||batch.enriched!==be||batch.consolidated!==Object.keys(redirects).length)fail('Estadísticas de la última revisión editorial desincronizadas');else ok(`Knowledge Expansion I: ${bp} promociones + ${be} enriquecimientos + ${Object.keys(redirects).length} consolidaciones`);
  ...
  const discoverySet=new Set(rawDiscovery.map(e=>e.id));for(const id of discoverySet)if(!shardSeen.has(id))fail(`Entrada externa sin shard: ${id}`);for(const id of shardSeen)if(!discoverySet.has(id))fail(`Shard contiene ID que no pertenece a la capa externa: ${id}`);if(shardSeen.size===discoverySet.size)ok(`${shardSeen.size} entradas externas cubiertas por shards`);
  for(const key of shardKeys){for(const e of inflate(shards[key]?.rows,shards[key]?.fields||index.entityFields)){const full=fullEntities.find(x=>x.id===e.id);if(full&&e.knowledgeStatus!==knowledgeStatus(full))fail(`Knowledge status desincronizado en shard ${key}: ${e.id}`);if(full&&Number(e.completeness)!==completeness(full))fail(`Completitud desincronizada en shard ${key}: ${e.id}`)}}
...
for(const stale of ['Research Encyclopedia v7','Research Expansion 7.0','404 · RESEARCH 7.0','44 paquetes culturales','GLOBAL RESEARCH 8.0','Global Research 8.0','GLOBAL RESEARCH 8.1','Global Research 8.1 ·'])if(html.includes(stale))fail(`Texto obsoleto visible: ${stale}`);
